...
{
    #if UNITY_EDITOR
    public class TMP_TextInfoDebugTool : EditorWindow
    {
        // Settings
        ...
        [MenuItem("Window/TextMeshPro/Text Info Debug Tool")]
        public static void ShowWindow()
        {
            GetWindow<TMP_TextInfoDebugTool>("Text Info Debug Tool");
        }
        ...
        void OnGUI()
        {
            ...
            ShowCharacters = EditorGUILayout.Toggle("Show Characters", ShowCharacters);
            ShowWords = EditorGUILayout.Toggle("Show Words", ShowWords);
            ShowLinks = EditorGUILayout.Toggle("Show Links", ShowLinks);
            ShowLines = EditorGUILayout.Toggle("Show Lines", ShowLines);
            ShowMeshBounds = EditorGUILayout.Toggle("Show Mesh Bounds", ShowMeshBounds);
            ShowTextBounds = EditorGUILayout.Toggle("Show Text Bounds", ShowTextBounds);

            GUILayout.Space(10);
            GUILayout.Label("Object Statistics", EditorStyles.boldLabel);
            EditorGUILayout.TextArea(ObjectStats, GUILayout.Height(100));
        }
        ...
        void OnSceneGUI(SceneView sceneView)
        {
            ...
            m_TextComponent = selected.GetComponent<TMP_Text>();
            ...
            string newStats = "Characters: " + m_TextInfo.characterCount + "   Words: " + m_TextInfo.wordCount + "   Spaces: " + m_TextInfo.spaceCount + "   Sprites: " + m_TextInfo.spriteCount + "   Links: " + m_TextInfo.linkCount
                          + "\nLines: " + m_TextInfo.lineCount + "   Pages: " + m_TextInfo.pageCount;

            if (ObjectStats != newStats)
            {
                ObjectStats = newStats;
                Repaint(); // Repaint the EditorWindow
            }

            // Get the handle size for drawing
        ...
        void DrawCharactersBounds()
        {
            ...
            for (int i = 0; i < characterCount; i++)
            {
                ...
                if (m_HandleSize < 0.5f && style != null)
                {
        ...
        {
            Handles.color = color;
            // Originally this forced Z to 0, which seems incorrect for 3D text.
            // We use BL.z for the Z plane assuming planar text.
            float z = BL.z;

            Handles.DrawLine(new Vector3(BL.x, BL.y, z), new Vector3(BL.x, TR.y, z));
            Handles.DrawLine(new Vector3(BL.x, TR.y, z), new Vector3(TR.x, TR.y, z));
            Handles.DrawLine(new Vector3(TR.x, TR.y, z), new Vector3(TR.x, BL.y, z));
            Handles.DrawLine(new Vector3(TR.x, BL.y, z), new Vector3(BL.x, BL.y, z));
        }
